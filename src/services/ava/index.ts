...
/**
 * Message interface for AVA chat
 */
...
/**
 * Get an initial greeting from the AVA agent
 * 
 ...
 * @returns Promise with a placeholder AVA response
 */
export const getInitialGreeting = async (
  user: User | null
): Promise<AvaResponse> => {
  console.log('AVA agent is being rebuilt - returning placeholder greeting');
...
/**
 * Check if the AVA agent is available
 * @returns Promise that resolves to false as the agent is being rebuilt
 */
export const checkAvaHealth = async (): Promise<boolean> => {
  console.log('AVA agent is being rebuilt - health check returning false');
  return false;
};

/**
 * Send a message to the AVA agent
 * 
 * This function sends the message and style information to the n8n webhook
 * and returns a placeholder response for the UI.
 * 
...
export const sendMessage = async (
  ...
  console.log('Sending message to AVA webhook', { messageLength: message?.length || 0, style });
  ...
  try {
    ...
    const webhookUrl = import.meta.env.VITE_AVA_WEBHOOK_URL;
    
    if (!webhookUrl) {
      throw new Error('AVA webhook URL not configured. Please check environment variables.');
    }
    ...
    try {
      ...
      return {
        output: "There was an error processing the response from AVA. Technical details: " + parseError.message,
        conversation_id: convoId
      };
    }
  } catch (error) {
      ...
      search_results: [
        {
          title: "Error",
          url: "#",
          description: "Could not connect to the AVA service. Please check your connection and try again."
        }
      ]
    };
  }
...
/**
 * Get the most recently updated AVA conversation for this user, if any
 * @param user - The authenticated user
 */
