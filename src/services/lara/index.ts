...
/**
 * LARA API response interface
 */
export interface LaraResponse {
  output: string;
...
/**
 * LARA API request interface
 */
export interface LaraRequest {
  message: string;
  style: {
    name: string;
    niche: string;
...
/**
 * Message interface for LARA chat
 */
...
/**
 * Get an initial greeting from the LARA agent
 *
 ...
 * @returns Promise with a placeholder LARA response
 */
export const getInitialGreeting = async (
  user: User | null
): Promise<LaraResponse> => {
  console.log('LARA agent is being rebuilt - returning placeholder greeting');
  ...
  return {
    output: 'Welcome to LARA! The LinkedIn Automated Rewriting Assistant.',
    conversation_id: crypto.randomUUID()
  };
};

/**
 * Check if the LARA agent is available
 * @returns Promise that resolves to false as the agent is being rebuilt
 */
export const checkLaraHealth = async (): Promise<boolean> => {
  console.log('LARA agent is being rebuilt - health check returning false');
  return false;
};

/**
 * Send a message to the LARA agent
 *
 ...
 * @returns Promise with the LARA response
 */
export const sendMessage = async (
  ...
  console.log('Sending message to LARA webhook', { messageLength: message?.length || 0, style });
  ...
  try {
    ...
    const webhookUrl = import.meta.env.VITE_LARA_WEBHOOK_URL;

    if (!webhookUrl) {
      throw new Error('LARA webhook URL not configured. Please check environment variables.');
    }
    ...
    if (!response.ok) {
      ...
      console.error('Error from LARA webhook:', errorText);
      throw new Error(`LARA webhook returned error: ${response.status} ${response.statusText}`);
    }
    ...
    try {
      ...
      return {
        output: "There was an error processing the response from LARA. Technical details: " + parseError.message,
        conversation_id: convoId
      };
    }
  } catch (error: any) {
    console.error('Error sending message to LARA webhook:', error);

    // Check if it's a network error (likely webhook is down)
    if (error.message.includes('Failed to fetch') || error.message.includes('Network request failed')) {
      return {
        output: 'Sorry, the LARA service is currently unavailable. Please try again later.',
        conversation_id: convoId
      };
    }
...
/**
 * Get the most recently updated LARA conversation for this user, if any
 * @param user - The authenticated user
 */
