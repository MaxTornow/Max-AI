...
const LaraChat: React.FC = () => {
  ...
  const [messages, setMessages] = useState<LaraMessage[]>(() => {
    try {
      ...
      const savedMessages = localStorage.getItem('lara_messages');
      if (savedMessages) {
      ...
      localStorage.removeItem('lara_messages');
    }
    return [];
  });
  ...
  const [conversationId, setConversationId] = useState<string | undefined>(() => {
    try {
      return localStorage.getItem('lara_conversation_id') || undefined;
    } catch (e) {
  ...
  const [selectedStyleId, setSelectedStyleId] = useState<string | undefined>(() => {
    try {
      return localStorage.getItem('lara_selected_style_id') || undefined;
    } catch (e) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('lara_messages', JSON.stringify(messages));
    } catch (e) {
  ...
  useEffect(() => {
    try {
      if (conversationId) {
        localStorage.setItem('lara_conversation_id', conversationId);
      } else {
        localStorage.removeItem('lara_conversation_id');
      }
    } catch (e) {
  ...
  useEffect(() => {
    try {
      if (selectedStyleId) {
        localStorage.setItem('lara_selected_style_id', selectedStyleId);
      } else {
        localStorage.removeItem('lara_selected_style_id');
      }
    } catch (e) {
    ...
    const loadInitialGreeting = async () => {
      if (messages.length === 0 && user && historyChecked) {
        try {
          ...
          const greetingMessage: LaraMessage = {
            id: uuidv4(),
          ...
          const fallbackMessage: LaraMessage = {
            id: uuidv4(),
            content: 'Welcome to Lara Chat! How can I help you create viral content today?',
            role: 'assistant',
            timestamp: new Date(),
          };
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const assistantMessage: LaraMessage = {
        id: uuidv4(),
        content: response.output,
        role: 'assistant',
        timestamp: new Date(),
      ...
      const errorMessage: LaraMessage = {
        id: uuidv4(),
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="bg-white dark:bg-gray-800 border-b dark:border-gray-700 p-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">LARA Chat</h1>
