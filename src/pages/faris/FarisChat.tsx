...
const FarisChat: React.FC = () => {
  ...
  const [messages, setMessages] = useState<FarisMessage[]>(() => {
    try {
      // Try to load messages from localStorage
      const savedMessages = localStorage.getItem('faris_messages');
      if (savedMessages) {
      ...
      localStorage.removeItem('faris_messages');
    }
    return [];
  });
  ...
  const [conversationId, setConversationId] = useState<string | undefined>(() => {
    try {
      return localStorage.getItem('faris_conversation_id') || undefined;
    } catch (e) {
  ...
  const [selectedStyleId, setSelectedStyleId] = useState<string | undefined>(() => {
    try {
      return localStorage.getItem('faris_selected_style_id') || undefined;
    } catch (e) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem('faris_messages', JSON.stringify(messages));
    } catch (e) {
  ...
  useEffect(() => {
    try {
      if (conversationId) {
        localStorage.setItem('faris_conversation_id', conversationId);
      } else {
        localStorage.removeItem('faris_conversation_id');
      }
    } catch (e) {
  ...
  useEffect(() => {
    try {
      if (selectedStyleId) {
        localStorage.setItem('faris_selected_style_id', selectedStyleId);
      } else {
        localStorage.removeItem('faris_selected_style_id');
      }
    } catch (e) {
    ...
    const loadInitialGreeting = async () => {
      if (messages.length === 0 && user && historyChecked) {
        try {
          ...
          const greetingMessage: FarisMessage = {
            id: uuidv4(),
          ...
          const fallbackMessage: FarisMessage = {
            id: uuidv4(),
            content: 'Welcome to FARIS Chat! How can I help you create viral content today?',
            role: 'assistant',
            timestamp: new Date()
          };
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    const userMessage: FarisMessage = {
      id: uuidv4(),
      content: messageText,
      role: 'user',
      timestamp: new Date()
    ...
    try {
      ...
      // Add Faris's response to the conversation
      const assistantMessage: FarisMessage = {
        id: uuidv4(),
        content: response.output,
        role: 'assistant',
        timestamp: new Date(),
      ...
      const errorMessage: FarisMessage = {
        id: uuidv4(),
    ...
    <div className="flex flex-col h-full bg-white dark:bg-gray-900">
      ...
      <div className="bg-white dark:bg-gray-800 border-b dark:border-gray-700 p-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">FARIS Chat</h1>
