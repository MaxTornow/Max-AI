...
const LacyChat: React.FC = () => {
  ...
  const [messages, setMessages] = useState<LacyMessage[]>(() => {
    try {
      // Try to load messages from localStorage
      const savedMessages = localStorage.getItem('lacy_messages');
      if (savedMessages) {
      ...
      localStorage.removeItem('lacy_messages');
    }
    return [];
  });
  ...
  const [conversationId, setConversationId] = useState<string | undefined>(() => {
    try {
      return localStorage.getItem('lacy_conversation_id') || undefined;
    } catch (e) {
  ...
  const [selectedStyleId, setSelectedStyleId] = useState<string | undefined>(() => {
    try {
      return localStorage.getItem('lacy_selected_style_id') || undefined;
    } catch (e) {
  ...
  useEffect(() => {
    try {
      if (conversationId) {
        ...
        localStorage.removeItem('lacy_conversation_id');
      }
    } catch (e) {
  ...
  useEffect(() => {
    try {
      if (selectedStyleId) {
        localStorage.setItem('lacy_selected_style_id', selectedStyleId);
      } else {
        localStorage.removeItem('lacy_selected_style_id');
      }
    } catch (e) {
    ...
    const loadInitialGreeting = async () => {
      if (messages.length === 0 && user && historyChecked) {
        try {
          ...
          const fallbackMessage: LacyMessage = {
            id: uuidv4(),
            content: 'Welcome to Lacy Chat! How can I help you create viral content today?',
            role: 'assistant',
            timestamp: new Date(),
          };
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage: LacyMessage = {
        id: uuidv4(),
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="bg-white dark:bg-gray-800 border-b dark:border-gray-700 p-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">LACY Chat</h1>
